...
export function Card({ children, className = '' }: PropsWithChildren<{ className?: string }>) {
  return <section className={`rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition hover:shadow-soft ${className}`}>{children}</section>;
}
...
export function Button({ className = '', variant = 'primary', ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: ButtonVariant }) {
  ...
  return (
    <button
      {...props}
      className={`rounded-xl px-4 py-2.5 text-sm font-semibold transition active:scale-[0.99] disabled:opacity-40 ${variantClass} ${className}`}
    />
  );
}
    ...
    <div className="grid grid-cols-2 gap-1 rounded-xl bg-gray-100 p-1 sm:grid-cols-4">
      {options.map((option) => (
        <button
          ...
          className={`rounded-lg px-3 py-2 text-xs font-medium ${value === option.value ? 'bg-white text-gray-900 shadow' : 'text-gray-600'}`}
        >
          {option.label}
        </button>
      ))}
